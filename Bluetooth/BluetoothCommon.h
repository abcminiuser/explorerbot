...
#define _BLUETOOTH_COMMON_H_
	...
	/* Enums: */
		...
		enum BT_ACL_ChannelStates_t
		{
			...
			ACL_CHANSTATE_WaitConnect           = 1, /**< A connection request has been received, but a response has not been sent. */
			ACL_CHANSTATE_WaitConnectRsp        = 2, /**< A connection request has been sent, but a response has not been received. */
			ACL_CHANSTATE_Config_WaitConfig     = 3, /**< Channel has been connected, but not yet configured on either end. */
			ACL_CHANSTATE_Config_WaitSendConfig = 4, /**< Channel configuration has been received and accepted, but not yet sent. */
			ACL_CHANSTATE_Config_WaitReqResp    = 5, /**< Channel configuration has been sent but not responded to, and a configuration
			                                          *   request from the remote end has not yet been received.
			                                          */
			ACL_CHANSTATE_Config_WaitResp       = 6, /**< Channel configuration has been sent but not accepted, but a configuration request
			                                          *   from the remote end has been accepted.
			                                          */
			ACL_CHANSTATE_Config_WaitReq        = 7, /**< Channel configuration has been sent and accepted, but a configuration request
			                                          *   from the remote end has not yet been accepted.
			                                          */
			ACL_CHANSTATE_Open                  = 8, /**< Channel is open and ready to send or receive data */
			ACL_CHANSTATE_WaitDisconnect        = 9, /**< A disconnection request has been sent, but not yet acknowledged. */
		};
	...
	/* User Implemented Callback Functions: */
		 ...
		 *
		 *  \param[in, out] StackState        Pointer to a Bluetooth Stack state table.
		 *  \param[in, out] ConnectionHandle  Pointer to the incoming connection information.
		 *
	...
	/* User Implemented Callback Event Functions: */
		 ...
		 *
		 *  \param[in, out] StackState        Pointer to a Bluetooth Stack state table.
		 *  \param[in, out] ConnectionHandle  Pointer to the established device connection information.
		 */
		 ...
		 *
		 *  \param[in, out] StackState        Pointer to a Bluetooth Stack state table.
		 *  \param[in, out] ConnectionHandle  Pointer to the closed device connection information.
		 */
